...
| Arithmetic modes | Keep strict host-specific rounding available; use native-f32 filter/demodulation in release WGPU by default, with WGSL validation | Put production throughput on the GPU while retaining a strict diagnostic/parity mode |
...
1. **Hardware acceptance gate.** Run the benchmark on representative discrete and integrated GPUs, record adapter/driver/CPU, and measure the release native-f32 plus block-IIR path. Repeat parity matrices on each driver. Small frames may remain faster on CPU; choose any crossover policy from measurements.
